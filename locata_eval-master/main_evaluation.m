...
flag_save_data = 0;         % if 1, the results are saved
flag_baseline = 1;          % if 1, baseline evaluation results are appended to the submissions
...
if load_data_flag
    ...
    load_data_fname = '../data/p_data_and_GT_2020-09-03_182315.mat';
else
    % Folder with the submission results of the participants
    p_dir = '../../databases/Submissions_anonymized';
    
    % Folder with final Eval database with all ground-truth data
    eval_dir = '../../databases/eval';
    
    % Folder with ground-truth VAD labelling for eval database
    eval_vad_dir = eval_dir;
end

% Directory containing any new results to be included for evaluation, 
% that were not submitted to the LOCATA challenge:
...
for ID = ids %#ok - ids saved and loaded from load_data_fname  (see l.115)
    ...
    for task_cnt = 1:length(tasks)
        ...
        if flag_save_data 
            results_task_dir = [results_dir, filesep,'ID_',num2str(ID), filesep ,'task', num2str(task_idx)];
        ...
        for rec_cnt = 1:length(recordings)
            ...
            for arr_cnt = 1:length(array_id)
                ...
                if ~isempty( estimates{ID, arr_idx, rec_idx, task_idx} ) && ~isempty(truth{ arr_idx_tr, rec_idx, task_idx})     %#ok - estimates & truth saved and loaded from load_data_fname  (see l.115)
                    ...
                    figure(all_measures{ID,arr_idx,rec_idx,task_idx}.f_ref); 
                    suptitle(['Task ', num2str(task_idx), ', Rec ', num2str(rec_idx), ', ', array_names{arr_idx}, ', ID ', num2str(ID) ]);
                    if flag_save_data
...
% Check whether results for plots are saved before
for i=1:length(valid_submission_ids)
    
    if ~exist( [results_dir,filesep,'ID_',num2str( valid_submission_ids(i) ) ] )
        warning('Figures not generated as directory with results not found.')
        return
    end   
end

%% Edit plot for Task 3, Recording 2, Submissions 6 + 7:
fig_327_em = openfig([results_dir, '/ID_7/task3/recording2/Eigenmike/measures_presentation_az.fig']);
obj_327_em_markers = fig_327_em.Children(2).Children(2);
obj_327_em_dots = fig_327_em.Children(2).Children(3);
pause(.1)
fig_326_em = openfig([results_dir, '/ID_6/task3/recording2/Eigenmike/measures_presentation_az.fig']);
hold on; 
...
%% Edit plot for Task 3, Recording 2, Submissions 6 + 7:
fig_347_robot = openfig([results_dir, '/ID_7/task3/recording4/Robot_head/measures_presentation_az.fig']);
fig_346_em = openfig([results_dir, '/ID_6/task3/recording4/Eigenmike/measures_presentation_az.fig']);
fig_343_dicit = openfig([results_dir, '/ID_3/task3/recording4/DICIT/measures_presentation_az.fig']);
...
fname = 'ID_4/task2/recording5/Robot_head/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_4/task2/recording5/Robot_head/measures_presentation_VAD';
rm_VAD_legend(results_dir, fname, 1)

fname = '/ID_4/task4/recording4/Robot_head/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_4/task4/recording4/Robot_head/measures_presentation_VAD';
rm_VAD_legend(results_dir, fname, 1)

fname = 'ID_4/task6/recording2/Robot_head/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_4/task6/recording2/Robot_head/measures_presentation_VAD';
rm_VAD_legend(results_dir, fname, 1)

% Fig 11:
fname = 'ID_4/task4/recording1/Robot_head/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_2/task4/recording1/Eigenmike/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_4/task4/recording1/Robot_head/measures_presentation_OSPA';
rm_VAD_legend(results_dir, fname)

fname = 'ID_2/task4/recording1/Eigenmike/measures_presentation_OSPA';
rm_VAD_legend(results_dir, fname)

fname = 'ID_2/task4/recording1/Robot_head/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_10/task4/recording1/Eigenmike/measures_presentation_az';
rm_VAD_legend(results_dir, fname)

fname = 'ID_2/task4/recording1/Robot_head/measures_presentation_OSPA';
rm_VAD_legend(results_dir, fname)

fname = 'ID_10/task4/recording1/Eigenmike/measures_presentation_OSPA';
rm_VAD_legend(results_dir, fname)

fname = 'ID_4/task4/recording1/Robot_head/measures_presentation_VAD';
rm_VAD_legend(results_dir, fname, 1)

fname = 'ID_2/task4/recording1/Eigenmike/measures_presentation_VAD';
rm_VAD_legend(results_dir, fname, 1)
